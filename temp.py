...
class SavePlusUI(MayaQWidgetDockableMixin, QMainWindow):
    ...
    OPT_VAR_HISTORY_EXPANDED = "SavePlusHistoryExpanded"
    ...
    def __init__(self, parent=None):
        try:
            ...
            self.container_layout.setSpacing(10)  # Consistent spacing between sections
            ...
            self.recent_files_section = ZurbriggStyleCollapsibleFrame("Recent Files", collapsed=True)
            
            # Create recent files content
            recent_files = QWidget()
            recent_files_layout = QVBoxLayout(recent_files)
            ...
            self.recent_files_section.add_widget(recent_files)
            self.container_layout.addWidget(self.recent_files_section)
            
            # Add recent_files_section toggled signal connection
            self.recent_files_section.toggled.connect(self.adjust_window_size)
            ...
            # Add a fixed-height spacer widget instead of a stretchable spacer
            spacer = QWidget()
            spacer.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
            spacer.setMinimumHeight(20)  # Provides some padding at the bottom
            ...
            self.pref_history_expanded = QCheckBox("Recent Files section expanded by default")
            ui_layout.addRow("", self.pref_history_expanded)
    ...
    def adjust_window_size(self):
        ...
        try:
            ...
            # Get current positions of sections before resize
            file_options_pos = self.file_options_section.pos()
            name_gen_pos = self.name_gen_section.pos()
            recent_files_pos = self.recent_files_section.pos()
            log_pos = self.log_section.pos()
            
            # Calculate total height needed with FIXED spacing between sections
            total_height = 0
            
            # Add heights for visible sections with fixed spacing
            if hasattr(self, 'file_options_section'):
                section_height = 30  # Header height
                if not self.file_options_section.is_collapsed():
                    section_height = 150  # Expanded height
                total_height += section_height
                total_height += 10  # Fixed spacing
            
            if hasattr(self, 'name_gen_section'):
                section_height = 30  # Header height
                if not self.name_gen_section.is_collapsed():
                    section_height = 280  # Expanded height
                total_height += section_height
                total_height += 10  # Fixed spacing
            ...
            if hasattr(self, 'recent_files_section'):
                section_height = 30  # Header height
                if not self.recent_files_section.is_collapsed():
                    section_height = 200  # Expanded height
                total_height += section_height
                total_height += 10  # Fixed spacing
            ...
            # Set the container widget's minimum height with precision
            self.container_widget.setMinimumHeight(total_height)
            
            # Ensure sections stay at their original positions
            QtCore.QCoreApplication.processEvents()
            
            if hasattr(self, 'file_options_section') and not self.file_options_section.is_collapsed():
                self.file_options_section.move(file_options_pos)
                
            if hasattr(self, 'name_gen_section') and not self.name_gen_section.is_collapsed():
                self.name_gen_section.move(name_gen_pos)
                
            if hasattr(self, 'recent_files_section') and not self.recent_files_section.is_collapsed():
                self.recent_files_section.move(recent_files_pos)
                
            if hasattr(self, 'log_section') and not self.log_section.is_collapsed():
                self.log_section.move(log_pos)
    ...
    def save_preferences(self):
        ...
        try:
            ...
            cmds.optionVar(iv=(self.OPT_VAR_HISTORY_EXPANDED, int(self.pref_history_expanded.isChecked())))
    ...
    def load_preferences(self):
        ...
        try:
            ...
            if cmds.optionVar(exists=self.OPT_VAR_HISTORY_EXPANDED):
                history_expanded = bool(cmds.optionVar(q=self.OPT_VAR_HISTORY_EXPANDED))
                self.pref_history_expanded.setChecked(history_expanded)
    ...
    def apply_ui_settings(self):
        ...
        try:
            ...
            history_expanded = self.pref_history_expanded.isChecked()
            ...
            if self.recent_files_section.is_collapsed() == history_expanded:
                self.recent_files_section.toggle_content()
...
def save_plus_proc(file_path=None):
    ...
    if assignment_match:
        ...
        last_name =assignment_match = re.match(r'([A-Z])(\d+)_([^_]+)_([^_]+)_([^_]+)_(\d+)$', base_name)
