...
class SavePlusUI(MayaQWidgetDockableMixin, QMainWindow):
    ...
    # Storage for singleton instance
    _instance = None
    ...
    def __init__(self, parent=None):
        try:
            ...
            if SavePlusUI._instance is not None:
                debug_print("SavePlusUI instance already exists! Raising the existing window.")
                SavePlusUI._instance.show()
                SavePlusUI._instance.raise_()
                return
                
            super(SavePlusUI, self).__init__(parent)
            SavePlusUI._instance = self
            ...
            # Add version label to status bar instead of header
            ...
            # Create collapsible sections for History tab
            ...
            # Create Recent Files section (expanded by default)
            self.recent_files_section = ZurbriggStyleCollapsibleFrame("Recent Files", collapsed=False)
            ...
            self.recent_files_section.add_widget(recent_files_content)
            self.history_container_layout.addWidget(self.recent_files_section)
            
            # Create Version History section
            self.version_history_section = ZurbriggStyleCollapsibleFrame("Version History", collapsed=False)
            
            version_history_content = QWidget()
            version_history_layout = QVBoxLayout(version_history_content)
            ...
            self.version_history_section.add_widget(version_history_content)
            self.history_container_layout.addWidget(self.version_history_section)
            
            # Add both sections to history tab through scroll area
            history_scroll_area = QScrollArea()
            history_scroll_area.setWidgetResizable(True)
            history_scroll_area.setWidget(self.history_container)
            history_scroll_area.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
            self.history_layout.addWidget(history_scroll_area)
            ...
            # Reset the singleton instance in case of error
            SavePlusUI._instance = None
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            # Remove the singleton instance reference
            SavePlusUI._instance = None
...
def save_plus_proc(file_path=None):
    ...
    assignment_match = re.match(r'([A-Z])(\d+)_([^_]+)_([^_]+)_([^_]+)_(\d+)"""
SavePlus - File versioning tool for Maya 2025 (Python 3 with PySide6)
Maya 2025 - Python 3 and PySide6 Compatible Version
"""
...
# Function to check for existing UI instances and show SavePlus UI
def show_saveplus_ui():
    # If an instance already exists, it will be returned via the constructor
    save_plus_ui = SavePlusUI()
    
    # If the constructor returned None, that means it detected an existing instance
    # and raised that instance instead of creating a new one
    if save_plus_ui is not None:
        save_plus_ui.show()
        save_plus_ui.raise_()
